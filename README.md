**Creating README without version references**

The user requests a README without version numbers. They’ve shared a README that includes "Version: 3.1" and changelog entries with specific versions, which I need to clean up.

I’ll ensure to follow the formatting rules by using Markdown headings, lists, and keeping bold text to a minimum as required by the developer. I also need to create a concise README while removing any version numbers while preserving the content.

Let’s get started on cleaning up the README!# Universal Quota & Folder Manager (UQFM)  
...
## Requirements
- Root privileges (script exits if not run as root)  
...
- Back up critical data/configuration before changing quotas.  
...
- Make /data shared for group devs with inherited group:
  - Start script → Smart folder permissions → Group shared + inherit → enter group devs → folder path /data
...
## Changelog (selected)
- Improved UX, tmpfs per-user, smart folder permissions, consolidated menu.  
- XFS project and Btrfs qgroup helpers, per-user tmpfs, backups for /etc/*.  
- Initial interactive release (quotas for ext*, XFS, Btrfs).
